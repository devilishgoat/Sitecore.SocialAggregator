...
namespace Sitecore.SocialAggregator
{
    public abstract class Source
    {
        ...
        protected abstract IList<Entry> DownloadData();
            ...
            if (root != null && entries.Any())
            {
                using (new SecurityDisabler())
                {
                    ...
                    foreach (var entry in entries)
                    {
                        ...
                        {
                            Log.Error(string.Format(CultureInfo.InvariantCulture, "Trying to create item for {0} source but the external id is empty", GetType().FullName), this);
                        }
                    }
                }
            }
